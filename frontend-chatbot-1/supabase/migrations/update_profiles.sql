-- Xóa trigger hiện có nếu có
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Xóa function hiện có nếu có
DROP FUNCTION IF EXISTS public.handle_new_user();

-- Xóa các policy hiện có
DROP POLICY IF EXISTS "Allow public read access" ON public.profiles;
DROP POLICY IF EXISTS "Allow individual update access" ON public.profiles;
DROP POLICY IF EXISTS "Allow individual insert access" ON public.profiles;

-- Xóa bảng profiles hiện có nếu có
DROP TABLE IF EXISTS public.profiles;

-- Tạo lại bảng profiles đơn giản để lưu trữ thông tin người dùng
